...
pub enum Error {
    ...
    #[error ("RegisterTemplateViaNameAndContentFileIoError ➡ {0:?}")]
    RegisterTemplateViaNameAndContentFileIoError(std::io::Error),
...
mod tests {
    ...
    pub static DIR: Lazy<PathBuf> = Lazy::new(||
        ...
        .join("src")
        .join("templater")
        .join("templater_with_handlebars")
    );
    ...
    fn test_register_template_via_name_and_content_file() {
        let mut templater = TemplaterX::new();
        let name = "alfa";
        let content_file = DIR
            .join("register_template_via_name_and_content_file")
            .join("template.html");
        assert!(content_file.exists());
        assert!(!templater.contains_template_name(name));
        templater.register_template_via_name_and_content_file(
            String::from(name),
            &content_file
        ).expect("register_template_via_name_and_content_file");
        assert!(templater.contains_template_name(name));
    }
